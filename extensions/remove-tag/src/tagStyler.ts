...
export default class TagStyler {
  private get config(): TagStylerConfig {
    return config.styles || { opening: { name: { underline: '#fff' } } }
  }
  ...
  public decoratePair = (pair: hmt.Match, editor: vscode.TextEditor) => {
    ...
    if (pair.opening.start !== pair.closing.start) {
      this.decorateTag(
        pair.closing,
        this.config.closing || this.config.opening,
        editor,
        false,
      )
    }
  ...
  private decorateTag(
    ...
    if (decorations.right) {
      this.applyDecoration(
        editor,
        decorations.right,
        new vscode.Range(end.translate(0, -1), end),
      )
    }
  }
  ...
  private applyDecoration(
    ...
    const isSelected =
      !selectionRange.isEmpty && !!range.intersection(selectionRange)
