...
export function findMatchingTag(
  ...
  for (let i = tagsList.length - 1; i >= 0; i -= 1) {
    ...
    if (
      (positionInName && matchFromName) ||
      (positionInAttributes && matchFromAttributes)
    ) {
      return {
...
export function getTagsForPosition(
  tagsList: hmt.PartialMatch[],
  position: number,
) {
  const x = tagsList.filter(
    pair =>
      isTagPairValid(pair) &&
      position > pair.opening!.start! &&
      position < pair.closing!.end!,
  ) as hmt.Match[]
